...
// ──────────────────────────────────────────────────────────────────────────
// sendNoteToGroup — fans a note out to every current member of a group.
// ──────────────────────────────────────────────────────────────────────────
export async function sendNoteToGroup(groupId: string, body: string): Promise<ActionResult> {
    try {
        const supabase = await createSupabaseServerClient();
        ...
        const menteeIds = (members ?? []).map((m) => m.mentee_id);

        if (menteeIds.length === 0) {
            return { success: false, message: "This group has no mentees yet." };
        }

        const { error } = await supabase
            .from("mentor_notes")
            .insert(menteeIds.map((menteeId) => ({ mentor_id: me.id, mentee_id: menteeId, body: trimmed })));
        ...
        return { success: true, message: `Note sent to ${menteeIds.length} mentee${menteeIds.length === 1 ? "" : "s"}.` };
    } catch (error) {
